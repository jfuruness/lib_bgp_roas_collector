            ...
            try:
                ...
                if not str(error_desc) == '1':
                    ...
                    tb_to_re = [x for x in str(traceback.format_tb(tb))
                        .split("File") if "lib_bgp_roas_collector" in x][-1]
...
class Logger:
    ...
    def _make_dir(self, path):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
            # Some idiot screwed up permimssions on our system, ergo...
            os.chmod(path, 0o777)
